...
def evaluate_combat(combat):
    try:
        ...
        if shipA.hull == 0:
            ...
            winner = shipA
        print("Finished {} x {} - Winner : {}".format(shipA.get_ship_fullname_weapons_utility(),
                                                      shipB.get_ship_fullname_weapons_utility(), winner.get_ship_fullname_weapons_utility()))
        return Winner([winner.get_ship_fullname_weapons_utility(), time, winner.hull, winner.armor, winner.shield])
...
def main():
    ...
    i = 0
    try:
        with Pool(initializer=init_worker, initargs=(ship_dict, weapon_dict, utility_dict), processes=cores) as pool:
            winner_list = pool.map(evaluate_combat,
                                   ship_comb, chunksize=len(ship_comb)//cores)
